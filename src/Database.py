...
def set_table(table_name: str, metadata: MetaData) -> Table:
    ...
    return Table(
        ...
        Column("conversionType", TEXT, nullable=False),
        Column("conversionSymbol", TEXT, nullable=False)
    )
...
class Database:
    ...
    def ensure_table_exists(self, table_name: str) -> None:
        ...
        table = set_table(self.table_name, self.metadata)
        ...
        try:
            ...
            logger.error(f"Failed to ensure table '{table_name}'", exc_info=True)
            raise
    ...
    def load_data(self, df: DataFrame, mode: str) -> None:
        
        try:            
            
            df.to_sql(self.table_name, self.engine, if_exists=mode, index=False)
            logger.info(f"{len(df)} rows loaded into '{self.table_name}' (mode={mode}).")
        
        except Exception as e:
        
            logger.error(f"Failed to load data into '{self.table_name}'", exc_info=True)
            raise
