WITH price_kpis AS (
    ...
    FROM {{ ref('fct_crypto_weekly') }}
    GROUP BY date, calendar_year, calendar_week, high, low, open, close, volume_from, volume_to
...
market_kpis AS (
    ...
    FROM {{ ref('fct_crypto_weekly') }}
    GROUP BY date, calendar_year, calendar_week, high, low, open, close
