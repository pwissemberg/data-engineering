WITH price_kpis AS (
    ...
    FROM {{ ref('fct_crypto_monthly') }}
    GROUP BY date, calendar_year, calendar_month, high, low, open, close, volume_from, volume_to
...
market_kpis AS (
    ...
    FROM {{ ref('fct_crypto_monthly') }}
    GROUP BY date, calendar_year, calendar_month, high, low, open, close
